...
def scale_until_timeout(runner: run.Runner, filename, command_num, sig_num, scopes=range(1, 21), timeout_s=60, cpu_time=True):
...
def scale_all_sigs(runner: run.Runner, filename, command_num, scopes=range(1, 21), timeout_s=60, cpu_time=True):
    ...
    for sig_num in range(1, num_sigs+1):
        yield from scale_until_timeout(
            runner, filename, command_num, sig_num, scopes=scopes, timeout_s=timeout_s, cpu_time=cpu_time)
